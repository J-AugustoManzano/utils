 ...
 *
 * Features:
 * - CRT-free, no C/C++ standard library
 ...
 *
 * Original author: Christopher Wellons
 * https://github.com/skeeto/scratch/blob/master/windows/cal.c
 *
 * Original program written in C.
 * Translation and adaptation to the C++ language by Augusto Manzano
 * Bug fix related to the transition date from the Julian to Gregorian calendar.
 *
 */
 ...
 */static void os_initlocale(struct locale *l) {
  int i, j;
  SYSTEMTIME t;
  GetSystemTime(&t);
  l->year = t.wYear;
...
static int os_write(int fd, const wchar_t *buf, int len) {
  int i;
  HANDLE h;
  DWORD tmp;
  int ulen = 0;
 ...
 */
static struct arg os_arg(struct arg a) {
  if (!a.beg) {
...
static int yearlen(int year) {
  return 1 + (year > 9) + (year > 99) + (year > 999);
}

/* Return true if the year is a leap year. */
static int isleap(int year) {
  return year <= 1582 ? (year % 4 == 0) /* Julian */
...
static int isgregorian(int year, int month, int day) {
  return year > 1582 || (year == 1582 && month > 10) || (year == 1582 && month == 10 && day > 4);
}
...
static wchar_t *pushchar(wchar_t *buf, wchar_t c) {
  buf[0] = c;
  return buf + 1;
}

static wchar_t *pushnewline(wchar_t *buf) {
  while (buf[-1] == ' ') {
...
static wchar_t *pushspace(wchar_t *buf, int len) {
  int i;
  for (i = 0; i < len; i++) {
    buf[i] = ' ';
  }
...
static wchar_t *pushstring(wchar_t *buf, wchar_t *s, int len) {
  int i;
  for (i = 0; i < len; i++) {
    buf[i] = s[i];
  }
...
static wchar_t *pushyear(wchar_t *buf, int year) {
  wchar_t *p = buf;
  switch (yearlen(year)) {
...
static wchar_t *pushdayline(wchar_t *buf, struct locale *l) {
  int i;
  for (i = 0; i < 7; i++) {
...
static wchar_t *pushmonthrow(wchar_t *buf, int year, int month, int y) {
  int x;
  int dow = zeller(year, month, 1) - 1;
  int ndays = monthdays(year, month);
  for (x = 0; x < 7; x++) {
    int i = y * 7 + x;
    if (year == 1582 && month == 10 && i + 1 - dow > 4) {
      i += 10; /* changeover from Julian to Gregorian */
    }
...
static wchar_t *pushsingle(wchar_t *buf, int year, int month, struct locale *l) {
  int y;
  wchar_t *p = buf;
...
/* Render a full year. */
static wchar_t *pushmulti(wchar_t *buf, int year, struct locale *l) {
  int y, my, mx;
  wchar_t *p = buf;
